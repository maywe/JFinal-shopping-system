 ...
 */
//初始化分页代码
function initPaginationData(pageId,service,parentModelId){
    ...
    if(pageNumber>1){
        htmlStr += '<li class="j-first"><a onclick="pagination(\''+pageId+'\','+totalPage+',1,\''+service+'\',\''+parentModelId+'\')" class="first" href="javascript:;"><i class="glyphicon glyphicon-step-backward font-size12"></i>首页</a></li>';
        htmlStr += '<li class="j-prev"><a onclick="pagination(\''+pageId+'\','+totalPage+','+(pageNumber-1)+',\''+service+'\',\''+parentModelId+'\')" class="previous" href="javascript:;"><i class="glyphicon glyphicon-backward font-size12"></i>上一页</a></li>';
    ...
    for(var i=1;i<=totalPage;i++){
        if(i==pageNumber){
            htmlStr += '<li class="active j-num"><a onclick="pagination(\''+pageId+'\','+totalPage+','+i+',\''+service+'\',\''+parentModelId+'\')" href="javascript:;">'+i+'</a></li>';
        }else{
    ...
    htmlStr += '<li class="j-goto"><span class="p-input goto-input"><input name="pageNumber" type="text" value="'+pageNumber+'"/></span><a onclick="pagination(\''+pageId+'\','+totalPage+',null,\''+service+'\',\''+parentModelId+'\')" class="goto" href="javascript:;" title="跳转"><i class="glyphicon glyphicon-chevron-right font-size12"></i></a></li></ul>';
    paginationBox.append(htmlStr);
}

//更新本页面
function pagination(pageId,totalPage,pageNumber,service,parentModelId){
    var pageObj = $('#'+pageId);
    //检查参数合法性
...
/* 查找带回分页 start*/
function lookupBackPaginationBreak(targetId,service,pageNumber,parentModelId){
    pagination(targetId,null,pageNumber,service,parentModelId);
}
/* 查找带回分页 end*/